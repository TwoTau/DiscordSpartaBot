...
bot.on("message", message => {
	...
	if (!sender.bot && message.channel.type === "text") {
		...
		if (command) {
			...
			if ((messageWords.includes("door") || messageWords.includes("room")) &&
				(messageWords.includes("can") || messageWords.includes("open") || messageWords.includes("lock") || messageWords.includes("locked"))) {
				const messageText = config.automated_message.open_door_message;
				message.reply(messageText);
			} else if (cleanContent.includes("www.amazon.com")) {
				message.reply("Consider using a smile.amazon.com link instead to donate 0.5% to a charity: " + message.cleanContent.trim().replace("www.amazon.com", "smile.amazon.com"));
			}
